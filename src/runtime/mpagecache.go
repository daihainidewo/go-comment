...
type pageCache struct {
	base  uintptr // 基址 // base address of the chunk
	cache uint64  // 标记是否空闲 1表示空闲 0表示使用 // 64-bit bitmap representing free pages (1 means free)
	scav  uint64  // 标记是否被清除 1表示清除 0表示未清除 // 64-bit bitmap representing scavenged pages (1 means scavenged)
}

// empty 表示是否有空闲可分配的 page
// empty reports whether the page cache has no free pages.
func (c *pageCache) empty() bool {
	return c.cache == 0
}

// alloc 申请 npages 页内存 返回内存页基地址和已清除内存字节数
...
// allocToCache 获取可能不是连续的 page 返回 pageCache
