...
func stopm() {
	...
	if _g_.m.locks != 0 { // 不能被锁
		throw("stopm holding locks")
	}
	if _g_.m.p != 0 { // 不能拥有p
		throw("stopm holding p")
	}
	if _g_.m.spinning { // 不能自旋
	...
	unlock(&sched.lock)
	mPark()
	acquirep(_g_.m.nextp.ptr()) // m与预留的p绑定
	_g_.m.nextp = 0
}
